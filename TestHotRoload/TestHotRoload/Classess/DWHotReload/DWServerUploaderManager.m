    ...
    //                                                               "__DATA", "__objc_classlist", &size );
                ...
                for (int i = 0; i < count; i++) {
                    ...
                    dispatch_async(dispatch_get_main_queue(), ^{
                        if([self.MyClass respondsToSelector:@selector(DWHotReload)]){
                            
                            SEL sel = NSSelectorFromString(@"DWHotReload");
                            IMP imp = [self.MyClass methodForSelector:sel];
                            void (*function)(void) = (void *)imp;
                            function();
                        }else{
                            [self restartApp];
                        }
...
-(void)restartApp{
    ...
    dispatch_after(dispatch_time(DISPATCH_TIME_NOW, 0.5 * NSEC_PER_SEC), dispatch_get_main_queue(), ^{
        if([delegate respondsToSelector:@selector(application:didFinishLaunchingWithOptions:)]){
            [delegate performSelector:@selector(application:didFinishLaunchingWithOptions:) withObject:nil];
        }
    });
}
